...
function setupPeers(channel, org, client) {
    for (let key in ORGS[org].peers) {
        let data = ORGS[org].peers[key]['tls_cacerts'];
...
function newRemotes(names, forPeers, userOrg) {
    ...
    for (let idx in names) {
        ...
        if (ORGS[userOrg].peers[peerName]) {
            ...
            let data = ORGS[userOrg].peers[peerName]['tls_cacerts'];
...
var getRegisteredUsers = function (username, userOrg, isJson) {
    ...
    var client = getClientForOrg(userOrg);
    var enrollmentSecret = null;
    logger.info("=======" + getKeyStoreForOrg(getOrgName(userOrg)));
    return hfc.newDefaultKeyValueStore({
        path: getKeyStoreForOrg(getOrgName(userOrg))
    }).then((store) => {
        client.setStateStore(store);
